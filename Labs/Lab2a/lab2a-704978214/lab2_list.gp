...
set key left top
plot \
     "< grep -e 'list-none-m' lab2_list.csv" using ($2):($7)/(($3)/4) \
	title '(adjusted) list w/mutex' with linespoints lc rgb 'blue', \
     "< grep -e 'list-none-s' lab2_list.csv" using ($2):($7)/(($3)/4) \
	title '(adjusted) list w/spinlock' with linespoints lc rgb 'green'