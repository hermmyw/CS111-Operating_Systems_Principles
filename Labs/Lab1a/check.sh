...
T9RC=$?
sleep 1
if [ $T9RC == 1 ] && [ ! -s test9out.txt ] && [ ! -s test9err.txt ] && [ ! -s c9out.txt ] && [ -s c9err.txt ]
then
  NUM_PASSED=`expr $NUM_PASSED + 1`
...
# Test case 14 --command with multiple arguments, correct input.
echo ""
echo "--->test case 14:"
touch test14in.txt
touch test14out.txt
